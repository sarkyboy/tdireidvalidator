...
import { Box, Container, Typography, Button, Paper, Grid, Modal, Snackbar, CircularProgress, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, Tooltip, IconButton, Divider, TextField, Link, Switch, FormControlLabel, FormGroup, Checkbox, TableContainer, Table, TableHead, TableRow, TableCell, TableBody, TableFooter, ToggleButtonGroup, ToggleButton, Menu, MenuItem } from '@mui/material';
...
const HISTORY_LOG_KEY = 'reid_tool_history_log';
const PROGRESS_KEY_PREFIX = 'reid_tool_progress_';
...
const StatItem = ({ icon, label, tooltipText, value, valueSx, onClick, onEdit, secondaryInfo }) => {
...
            return { showFolderInfo: false, autoSelectAll: false, ...parsed };
        }
    } catch (error) {
...
        if (checked) newSet.add(name); else newSet.delete(name);
        return newSet;
    });
  };
...
        }
        if (shouldProcess) uuidFolderHandlesToProcess.push(uuidHandle);
    }
...
        setProgress({ value: percentage, text: TEXTS.progressText(processedFoldersCount, totalFolders) });
    }
    
...
    return ((date2 - date1) / 60000).toFixed(2);
  };
...
          if (line.includes(modeInfo.startWord)) rawEvents.push({ type: 'start', time });
          else if (line.includes(modeInfo.endWord)) rawEvents.push({ type: 'end', time });
      });
...
          }
      }
      openStarts.forEach(startEvent => { pairedEvents.push({ startTime: startEvent.time, endTime: null }); });
...
                  if (!eventData[modeKey]) eventData[modeKey] = [];
...
            if (total > 0) totalDurations[modeKey] = total;
        }
    }
...
    
    return { folderUUID: dirHandle.name.split('_').pop(), folderPath: path, images: Array.from(imageMap.values()).sort((a, b) => a.uuid.localeCompare(b.uuid)), newPeople: [], eventData: eventData, totalDurations: totalDurations, isStaff: false };
  };
  
...
  const handleDurationFilterTypeChange = (event, newType) => { setFilters(prev => ({ ...prev, durationFilterType: newType || '' })); };
  const handleResetFilters = (defaultDurationType = '') => { setFilters({ durationLessThan: '', durationMoreThan: '', showFalseNegative: false, showFalsePositive: false, durationFilterType: defaultDurationType || (availableDurationFilters[0] || '') }); };
  const handleSaveMissedPeople = () => { setMissedPeopleCount(parseInt(missedPeopleInput, 10) || 0); setIsMissedPeopleDialogOpen(false); };
  const triggerToggleStaff = (folderUUID) => { const folder = folders.find(f => f.folderUUID === folderUUID); if (!folder) return; if (folder.isStaff) { setFolders(prev => prev.map(f => f.folderUUID === folderUUID ? { ...f, isStaff: false } : f)); } else { setStaffConfirmState({ isOpen: true, targetUUID: folderUUID }); } };
  const handleCloseStaffConfirm = () => { setStaffConfirmState({ isOpen: false, targetUUID: null }); };
  const handleConfirmHideStaff = () => { if(staffConfirmState.targetUUID) { setFolders(prev => prev.map(f => f.folderUUID === staffConfirmState.targetUUID ? { ...f, isStaff: true } : f)); } handleCloseStaffConfirm(); };
...
            if (filters.durationLessThan && (duration === undefined || duration >= parseFloat(filters.durationLessThan))) return false;
            if (filters.durationMoreThan && (duration === undefined || duration <= parseFloat(filters.durationMoreThan))) return false;
        }
        return true;
    });
...
            <InitialScreen onFolderSelect={handleSelectFolderClick} onDrop={handleDrop} onDragOver={handleDragOver} onDragLeave={handleDragLeave} isDragging={isDragging} disabled={isLoading} />
        ) : (
            <>
...
                            <StatItem icon={<CallMissedOutgoingIcon fontSize="large" />} label={TEXTS.missingExitEvents} tooltipText={TEXTS.missingExitEventsTooltip} value={missingExitFolders.length} onClick={() => startNavigation(missingExitFolders)} valueSx={{ color: missingExitFolders.length > 0 ? 'error.main' : 'text.primary' }} />
                          </>
                        )}
                      </Grid>
                    </Paper>
...
                                </>
                            )}
                            <Grid item xs={12} sm={6} md><FormControlLabel control={<Switch checked={filters.showFalseNegative} onChange={handleFilterChange} name="showFalseNegative" />} label={TEXTS.showOnlyFalseNegative} /></Grid>
                            <Grid item xs={12} sm={6} md><FormControlLabel control={<Switch checked={filters.showFalsePositive} onChange={handleFilterChange} name="showFalsePositive" />} label={TEXTS.showOnlyFalsePositive} /></Grid>
                        </Grid>
                    </Paper>
                  </>
                )}
...
                            const totalDuration = (modeKey === 'counting_only' || modeKey === 'dwelling_only') ? events.reduce((sum, event) => sum + (parseFloat(event.duration) || 0), 0) : 0;
                            return (
                              <Box key={modeKey} sx={{ mb: 1.5 }}>
                                  <Typography variant="subtitle2" gutterBottom>{modeInfo.title}:</Typography>
                                  <TableContainer component={Paper} variant="outlined"><Table size="small" aria-label={`${modeKey} events table`}>
                                  <TableHead><TableRow><TableCell>{modeInfo.startWord}</TableCell><TableCell>{modeInfo.endWord}</TableCell><TableCell align="right">Duration (min)</TableCell></TableRow></TableHead>
                                  <TableBody>{events.map((event, index) => ( <TableRow key={index} ><TableCell sx={{ color: event.isComplete ? 'inherit' : 'error.main' }}>{event.startTime}</TableCell><TableCell sx={{ color: event.isComplete ? 'inherit' : 'error.main' }}>{event.endTime}</TableCell><TableCell align="right" sx={{ color: event.isComplete ? 'inherit' : 'error.main' }}>{event.duration}</TableCell></TableRow> ))}</TableBody>
                                  {(modeKey === 'counting_only' || modeKey === 'dwelling_only') && totalDuration > 0 && ( <TableFooter><TableRow sx={{ '& th, & td': { fontWeight: 'bold', borderTop: `2px solid ${themeMode === 'dark' ? 'rgba(255, 255, 255, 0.23)' : 'rgba(0, 0, 0, 0.23)'}` } }}><TableCell colSpan={2} align="right">Total</TableCell><TableCell align="right">{totalDuration.toFixed(2)}</TableCell></TableRow></TableFooter> )}
...
        <Dialog open={isLoading} disableEscapeKeyDown><DialogContent sx={{ textAlign: 'center', p: 4 }}><CircularProgressWithLabel value={progress.value} /><DialogContentText sx={{ mt: 2 }}>{loadingMessage}</DialogContentText><DialogContentText variant="body2" sx={{ color: 'text.secondary', minHeight: '1.2em' }}>{progress.text}</DialogContentText></DialogContent></Dialog>
...
  const [themeMode, setThemeMode] = useState(() => { const savedTheme = localStorage.getItem('themeMode'); return savedTheme || 'dark'; });
